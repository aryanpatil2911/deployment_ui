...
async function loadData() {
    ...
    try {
        const res = await fetch(deploymentLogsIndexUrl); if (!res.ok) throw new Error();
        const files = await res.json(); deployments = [];
        for (const file of files) {
            if (!file.endsWith('.csv')) continue;
            const fRes = await fetch(`${deploymentLogsPath}/${file}`); if (!fRes.ok) continue;
            deployments.push(...parseCSV(await fRes.text()));
        }
